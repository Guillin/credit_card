...
def main(input_file, output_file):
    ...
    logger.info('INIT: build features engineer from processed data'+colors.RED)

    logger.info('RUN: loading data')
    #df = load_data(input_file, kind='csv')
    df = pd.read_csv(config.INPUT_PATH + input_file)
    print(df.head())
    ...
    corpus = [text for text in tqdm(df.iloc[:100, 1])]
    features, vect = text_feateng.get_bow(corpus)
    print(features.head())
    del df 
